...
def subsets(Set):
    """
    Find start index of subsets (series of consecutive numbers)
    """
    subsets = []
    start = 0
    end = 0
...
def get_trials(feature):
    """
    Find index of start of trial based on design matrix vector
    """
...
def split_data(data, labels, axis=-1, attr=1):
    ...
    The function partitions a multidimensional data array into smaller slices, based on the shapes provided in the labels list. The slices are then stored in a dictionary, where the keys are determined by the `attr` attribute of the info list.
    ...
    labels : list
        List of tuples, where each tuple contains an info list and a shape integer. Example: 
        [
         (['A07', 'Day1', 'ATC1'], 30000),
         (['A07', 'Day1', 'ATC2'], 30000)
        ]
    ...
    attr : int, optional
        The index of the attribute in the info list that will be used as a key in the resulting dictionary, by default 1 (the second item).

    Returns
    -------
    partitioned_data : dict
        A dictionary where keys are the extracted attribute from the info list, and values are the partitioned slices of the input data.
